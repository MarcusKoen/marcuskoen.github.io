---
author: Marcus Koen
title: Chip8 Emulator (In Progress)
---

<style>
html, body {
  margin: 0;
  padding: 0;
  background: #0f0f0f;
  color: #e0e0e0;
  font-family: monospace, system-ui, sans-serif;
}

/* Rain container */
.rain-container {
  position: fixed;
  inset: 0;
  pointer-events: none;
  z-index: 1;
  overflow: hidden;
}

/* Individual raindrop */
.raindrop {
  position: absolute;
  width: 1.5px;           /* slightly thicker looks better with color */
  height: 70px;
  background: linear-gradient(
    to bottom,
    transparent,
    hsl(0, 100%, 70%) 20%,    /* red   */
    hsl(60, 100%, 70%) 40%,   /* yellow*/
    hsl(120, 100%, 70%) 60%,  /* green */
    hsl(180, 100%, 70%) 80%,  /* cyan  */
    hsl(240, 100%, 70%)       /* blue → purple-ish at bottom */
  );
  animation: 
    rain 1.2s linear infinite,
    hue-rotate 6s linear infinite;
  opacity: 0.7;
  filter: brightness(1.3);   /* helps colors pop on dark bg */
}

/* Falling animation (same as before, just moved here for clarity) */
@keyframes rain {
  from {
    transform: translateY(-120vh) translateX(0) skewX(10deg);
  }
  to {
    transform: translateY(120vh) translateX(-60px) skewX(10deg);
  }
}

/* Makes colors cycle continuously */
@keyframes hue-rotate {
  from {
    filter: hue-rotate(0deg) brightness(1.3);
  }
  to {
    filter: hue-rotate(360deg) brightness(1.3);
  }
}
/* Variation */
.raindrop:nth-child(1)  { left: 5%;  animation-duration: 0.9s; }
.raindrop:nth-child(2)  { left: 10%; animation-duration: 1.1s; }
.raindrop:nth-child(3)  { left: 15%; animation-duration: 0.8s; }
.raindrop:nth-child(4)  { left: 20%; animation-duration: 1.2s; }
.raindrop:nth-child(5)  { left: 25%; animation-duration: 0.95s; }
.raindrop:nth-child(6)  { left: 30%; animation-duration: 1.05s; }
.raindrop:nth-child(7)  { left: 35%; animation-duration: 0.85s; }
.raindrop:nth-child(8)  { left: 40%; animation-duration: 1.15s; }
.raindrop:nth-child(9)  { left: 45%; animation-duration: 0.9s; }
.raindrop:nth-child(10) { left: 50%; animation-duration: 1.2s; }
.raindrop:nth-child(11) { left: 55%; animation-duration: 0.8s; }
.raindrop:nth-child(12) { left: 60%; animation-duration: 1.1s; }
.raindrop:nth-child(13) { left: 65%; animation-duration: 0.95s; }
.raindrop:nth-child(14) { left: 70%; animation-duration: 1.05s; }
.raindrop:nth-child(15) { left: 75%; animation-duration: 0.85s; }
.raindrop:nth-child(16) { left: 80%; animation-duration: 1.15s; }
.raindrop:nth-child(17) { left: 85%; animation-duration: 0.9s; }
.raindrop:nth-child(18) { left: 90%; animation-duration: 1.2s; }
.raindrop:nth-child(19) { left: 95%; animation-duration: 0.8s; }

  /* Fix code block visibility */
pre, code {
  background-color: #0b0b0b !important;
  color: #f2f2f2 !important;
}

/* Inline code */
code {
  padding: 0.15em 0.35em;
  border-radius: 4px;
}

/* Fenced code blocks */
pre {
  padding: 1em;
  overflow-x: auto;
  border-radius: 6px;
  box-shadow: inset 0 0 0 1px rgba(255,255,255,0.08);
}

/* Optional: monospace consistency */
pre, code {
  font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace;
}

</style>

<div class="rain-container">
  <div class="raindrop"></div><div class="raindrop"></div><div class="raindrop"></div>
  <div class="raindrop"></div><div class="raindrop"></div><div class="raindrop"></div>
  <div class="raindrop"></div><div class="raindrop"></div><div class="raindrop"></div>
  <div class="raindrop"></div><div class="raindrop"></div><div class="raindrop"></div>
  <div class="raindrop"></div><div class="raindrop"></div><div class="raindrop"></div>
  <div class="raindrop"></div><div class="raindrop"></div><div class="raindrop"></div>
  <div class="raindrop"></div>
</div>

# Repository link
Link for the repository can be found [here](https://github.com/MarcusKoen/Chip8-Emu)

# Specifications
1. 16 8-bit registers
2. 4096 bits total memory
3. PC should be 16-bit
4. The original implementation of the Chip-8 language includes 36 different instructions, including math, graphics, and flow control functions.
5. All instructions are 2 bytes long and are stored most-significant-byte first. In memory, the first byte of each instruction should be located at an even addresses.

Note to self, cannot switch in class declaration, only in functions.

# Stack Pointer Convention
sp points to next free slot
initial state sp=0;
stack is empty
next push goes to stack[0]

# Shifting
| Field          | Mask   | Shift |
| -------------- | ------ | ----- |
| X (register)   | 0x0F00 | >> 8  |
| Y (register)   | 0x00F0 | >> 4  |
| N (low nibble) | 0x000F | none  |
| KK (byte)      | 0x00FF | none  |
| NNN (address)  | 0x0FFF | none  |
